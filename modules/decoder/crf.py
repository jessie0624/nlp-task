...
def _get_encoding_type_from_tag_vocab(tag_vocab: Union[Vocabulary, dict])->str:
    ...
    :param tag_vocab: 支持传入tag Vocabulary, 或者dict{0:"0",1:"B-tag1"}
    :return
    """
    tag_set = set()
    unk_token = '<unk>'
...
def allowed_transitions(tag_vocab:Union[Vocabulary, dict], encoding_type=None,
                        include_start_end_trans=False):
    r"""
     给定一个id到label的映射表，返回所有可以跳转的(from_tag_id, to_tag_id)列表
     :param ~ tag_vocab:支持类型为 tag 或者 tag-label.
            只有tag的比如"B","M",也可以是tag-label:“B-NN”,"M-NN"
            如果传入dict,格式需要如:{0:"0",1:"B-tag1"} 即index在前，tag在后
     :param str encoding_type: 支持‘bio’,'bioes','bmeso','bmes'
     :pram bool include_start_end: 是否包含开始与结尾的转换。比如在bio中，b/o可以在开头，但是i不能在开头；
...
def _is_transition_allowed(encoding_type, from_tag, from_label, to_tag, to_label):
    ...
    BIOES: (Begin, Inside, Outside, End, Start)
    BMES: (Begin, Middle, End, Single)
    BMESO: (Begin, Middle, End, Single, Outer)
    :param str encoding_type: 支持'BIO', 'BMES', 'BEMSO','BIOSE';
    :param str from_tag: 比如"B","M"之类的标注tag.还包括start,end等两种特殊的tag
    :param str from_label: 比如“PRE”,'LOC'等label
    :param str to_tag: 如from_tag
    :param str to_label: 如from_label
    :return bool, 能否转移
    """
    ...
    if encoding_type == 'bio':
        r"""
        第一行是to_tag, 第一列是from_tag,
        y任意条件可转，-只有在label相同时可转，n不可转
        +-------+---+---+---+-------+-----+
        |       | B | I | O | start | end |
        +-------+---+---+---+-------+-----+
        |   B   | y | - | y | n     | y   |
        ...
        if from_tag == 'start':
            ...
            return any([to_tag in ['end', 'b', 'o'], 
                        to_tag == 'i' and from_label == to_label])
        elif from_tag == 'o':
...
class ConditionalRandomField(nn.Module):
    r"""
    条件随机场，提供forward()以及viterbi_decode()两个方法
    分别用于训练和inference
    """
    def __init__(self, num_tags, include_start_end_trans=False,
                allowed_transitions=None, initial_method=None):
        r"""
        :param int num_tags: 标签数量
        :param bool include_start_end_trans:是否考虑各个tag作为起始和结尾的分数
        :param List[Tuple[from_tag_id(int), to_tag_id(int)]],allwoed_transition:
                内部的Tuple[from_tag_id(int), to_tag_id(int)] 视为允许发生的跃迁，其他
                没有包含的跃迁认为是禁止转移的，可以通过allowed_transitions()函数得到，
                如果为None，则所有跃迁均为合法
        ...
        # transition matrix (from_tag_id, to_tag_id)
    ...
    def _normalizer_likelihood(self, logits, mask):
        r"""
        compute the （batch_size, ）denominator term for the log-likelihood,
    ...
    def _gold_score(self, logits, tags, mask):
        ...
        mask = mask.eq(True)
        flip_mask = mask.eq(False)
        # 获取trans_score 去除START, END
        # trans_score: [L-1,B], emit_score: [L, B], score:[L-1,B]
        ...
        if self.include_start_end_trans:
            ...
            score = score + st_scores + end_scores
    ...
    def forward(self, feats, tags, mask):
        ...
        :param torch.FloatTensor feats: batch_size * max_len * num_tags, 特征矩阵
        :param torch.LongTensor tags: batch_size * max_len, 标签矩阵
        :param torch.ByteTensor mask: batch_size * max_len, 为0的位置认为是padding
        :return: torch.FloatTensor, (batch_size, )
        """
        feats = feats.transpose(0, 1) # batch_first
    ...
    def viterbi_decode(self, logits, mask, unpad=False):
        ...
        :param torch.FloatTensor logits: batch_size * max_len * num_tags, 特征矩阵
        :param troch.ByteTensor mask: batch_size * max_len, 为0的位置认为是padding的位置
        :param bool unpad: 是否将结果删取padding. False,返回batch_size * max_len的tensor,
                        True，返回的是List[List[int]], 内部的List[int]为每个sequence的lable，
                        已经去除pad部分，即每个List[int]的长度是这些sample的真实有效长度
        ...
        if self.include_start_end_trans: # start 可以转移到其他tag，其他tag可以转移到end。
            transitions[n_tags, :n_tags] += self.start_scores.data
            transitions[:n_tags, n_tags+1] += self.end_scores.data 

        vscore += transitions[n_tags, :n_tags] # 加上开始的分值
        trans_score = transitions[:n_tags, :n_tags].view(1, n_tags, n_tags).data
        for i in range(1, seq_len):
            prev_score = vscore.view(batch_size, n_tags, 1)
            cur_score = logits[i].view(batch_size, 1, n_tags)
            score = prev_score + cur_score + trans_score
        ...
        ans_score, last_tags = vsocre.max(1)
